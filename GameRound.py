...
def filter_parameters(must_chars, not_chars, not_here_chars, position_chars):
    s = "Filter values: " + " Must chars [" + must_chars
    s += "] not_chars {" + not_chars
...
class GameRound:
    ...
    def get_guess(self, guesses, matches):
        ...
        Trace.write(filter_parameters(must_chars, not_chars, not_here_chars, position_chars))
        # Trace.write("Time to filter " + str(t1.stop()))
        Trace.write("Filtered answers " + list_to_str(filtered.words))
        if filtered.count() <= 2:
            Trace.write("Selecting from one or two words in answers " + str(filtered.count()))
        ...
        if len(guesses) >= 1:
            return guesses[0]
        Trace.write("@@@ No guess generated")
        return "ZZZZZ"
